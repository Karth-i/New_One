...
train_size = 0.8
train_data=new_df.sample(frac=train_size,random_state=50)
...
torch.save(model, '/content/drive/MyDrive/roberta_model.pth')
...
model = model2
# Define a class to process the chat history
class ChatHistoryProcessor:
    def __init__(self):
        self.user_messages = defaultdict(list)

    # Process a WhatsApp chat history file
            ...
            for line in lines:
                ...
                if len(parts) > 1:
                    ...
                    if len(user_parts) > 1:
                        ...
                        english_words = extract_english_words(message)
                        self.user_messages[user].extend(english_words)
        return self.user_messages

    # Extract English words from a message
    ...
    def process_file(self, file_path):
        self.user_messages = self.process_whatsapp_file(file_path)
...
# Process the WhatsApp chat history file
processor.process_file('/content/WhatsApp Chat.txt')

# Get the messages for a specific user
user_messages = processor.get_messages('Karthi')
